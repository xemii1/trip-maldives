...
4. **Цены на level.travel очень динамичные — сайт сам предупреждает «Цены обновятся через 30 минут»** и обновляет их автоматически. При первой проверке (21.08.2026, днём) 27 пакетных отелей показали одни цены; при повторной проверке (21.08.2026, вечером, тем же методом на те же даты) **20 из 27 цен изменились** — в среднем на 6-8%, у части отелей — заметно сильнее (Perla Dhangethi: 343 866 → 302 850 ₽, Canareef: 433 034 → 399 722 ₽, Royal Island: 552 580 → 512 290 ₽). Таблица ниже отражает **последнюю проверенную версию**, но эти цифры стоит воспринимать как снимок на конкретный момент времени, а не гарантированную цену — перед покупкой обязательно перепроверяйте на level.travel напрямую.
5. Из 27 отелей с подтверждённым «живым» тарифом «Всё включено» (пакетные туры level.travel) **8 укладываются в бюджет 500 000 ₽**: Perla Dhangethi (⚠️ без алкоголя), Canareef Resort Maldives, Adaaran Select Hudhuranfushi, Ellaidhoo Maldives by Cinnamon, Centara Ras Fushi Resort & Spa, Bandos Maldives, Kandima Maldives, Cinnamon Dhonveli Maldives.
6. Цена «Итого на двоих» в таблице ниже — это **реальная, подтверждённая вживую на level.travel цена всего пакета** (перелёт + отель + трансфер). Для пакетных вариантов (без 🅱️) в файлах `trips/*.md` **не показывается** отдельная «расчётная» цена перелёта и отдельная «расчётная» цена отеля — оба указаны как «входит в стоимость пакета», потому что туроператор не публикует такую разбивку, и любая попытка разделить единую цену была бы искусственной оценкой. Раздел «Рейс» по-прежнему приводит для конкретики реальный прямой рейс Аэрофлота (даты, время, номера рейсов, багаж) — это ориентир на случай, если понадобится похожий рейс, а не обязательно тот перевозчик, которым в реальности летит купленный тур.
7. **Перекрёстная проверка на Booking.com** (независимо от level.travel): Canareef Resort Maldives, тариф «Всё включено», 7 ночей, 2 взрослых, **только отель без перелёта — 220 358 ₽**. Того же порядка, что и рассчитанная доля отеля в пакете level.travel — подтверждает, что цифры в таблице ниже не завышены и не занижены на порядок (подробности — в файле варианта).

8. **Поиск напрямую на Booking.com (не только отели из исходного списка level.travel)** нашёл ещё **23 курортных острова** с официальным фильтром Meal Plan = «Всё включено» (это отдельная категория самого Booking.com, а не текст в описании — надёжнее, чем у level.travel), которых не было в исходной подборке. Из них **5 укладываются в бюджет**: **Adaaran Club Rannalhi** (424 095 ₽ отель + перелёт), **Riu Atoll** (426 609 ₽), **Meeru Maldives Resort Island** (447 158 ₽, рейтинг 9,2 при 976 отзывах), **OBLU XPERIENCE Ailafushi** (458 986 ₽), **Malahini Kuda Bandos** (485 326 ₽). Остальные **18** превышают бюджет, но добавлены для полноты картины и сравнения атоллов/типов курортов. Здесь цена отеля показана **отдельно от перелёта** (на Booking.com нет пакетных туров) — к цене отеля прибавлена та же ориентировочная стоимость перелёта (145 000 ₽), что и в остальных расчётах. Цены на Booking.com были проверены один раз (21.08.2026) и не перепроверялись повторно — тоже могли измениться.

**Вывод:** бюджет 500 000 ₽ на двоих за 7 ночей с настоящим «всё включено + алкоголь» на острове-курорте Мальдив в начале ноября 2026 — достижим: **12 настоящих курортных островов** укладываются в бюджет с гарантированным алкоголем (7 через пакетные туры level.travel + 5 через прямое бронирование на Booking.com). Perla Dhangethi формально тоже укладывается по цене, но алкоголь там, вероятнее всего, не входит — сохранён в списке как самый дешёвый бюджетный вариант, но с явной пометкой несоответствия требованию.

Вариантов, уложившихся в бюджет по цене: **13 из 50** проверенных, из них с подтверждённым алкоголем — **12**. Максимум в подборке — 808 590 ₽, минимум — 302 850 ₽ (полный пакет с перелётом, Perla Dhangethi). Таблица отсортирована по возрастанию итоговой цены.

## Список вариантов

| № | Отель | Тип предложения | Категория | Атолл | Итого на двоих (реальная цена) | В бюджете 500к | Файл |
|---|---|---|---|---|---|---|---|
| 1 | Perla Dhangethi ⚠️ без алкоголя | Пакет | 4* | Южный Ари | 302 850 ₽ | ✅ | [trip-perla-dhangethi-1.md](trips/trip-perla-dhangethi-1.md) |
| 20 | Canareef Resort Maldives | Пакет | 4* | Адду (южный атолл) | 399 722 ₽ | ✅ | [trip-canareef-resort-maldives-20.md](trips/trip-canareef-resort-maldives-20.md) |
...
| 27 | Adaaran Select Hudhuranfushi | Пакет | 4* | Северный Мале | 425 082 ₽ | ✅ | [trip-adaaran-select-hudhuranfushi-27.md](trips/trip-adaaran-select-hudhuranfushi-27.md) |
| 32 | Riu Atoll 🅱️ | 🅱️ Отдельно | 4* | Даалу | 426 609 ₽ | ✅ | [trip-riu-atoll-all-inclusive-32.md](trips/trip-riu-atoll-all-inclusive-32.md) |
| 19 | Ellaidhoo Maldives by Cinnamon | Пакет | 4* | Северный Ари | 433 653 ₽ | ✅ | [trip-ellaidhoo-maldives-by-cinnamon-19.md](trips/trip-ellaidhoo-maldives-by-cinnamon-19.md) |
...
| 7 | Centara Ras Fushi Resort & Spa (Adults Only 18+) | Пакет | 4* | Северный Мале | 465 461 ₽ | ✅ | [trip-centara-ras-fushi-resort-spa-adults-only-18-7.md](trips/trip-centara-ras-fushi-resort-spa-adults-only-18-7.md) |
| 16 | Bandos Maldives | Пакет | 4* | Северный Мале | 466 079 ₽ | ✅ | [trip-bandos-maldives-16.md](trips/trip-bandos-maldives-16.md) |
...
| 6 | Kandima Maldives | Пакет | 5* | Даалу | 491 880 ₽ | ✅ | [trip-kandima-maldives-6.md](trips/trip-kandima-maldives-6.md) |
...
| 13 | Royal Island Resort & Spa | Пакет | 5* | Атолл Баа (биосфера ЮНЕСКО) | 512 290 ₽ | ❌ | [trip-royal-island-resort-spa-13.md](trips/trip-royal-island-resort-spa-13.md) |
...
| 9 | Sheraton Maldives Full Moon Resort & Spa | Пакет | 5* | Северный Мале | 529 078 ₽ | ❌ | [trip-sheraton-maldives-full-moon-resort-spa-9.md](trips/trip-sheraton-maldives-full-moon-resort-spa-9.md) |
...
| 15 | Furaveri Island Resort & Spa | Пакет | 5* | Раа | 542 419 ₽ | ❌ | [trip-furaveri-island-resort-spa-15.md](trips/trip-furaveri-island-resort-spa-15.md) |
...
| 4 | Machchafushi Island Resort & Spa | Пакет | 4* | Южный Ари | 550 195 ₽ | ❌ | [trip-machchafushi-island-resort-spa-4.md](trips/trip-machchafushi-island-resort-spa-4.md) |
...
| 26 | Cocoon Maldives | Пакет | 5* | Лавияни | 560 532 ₽ | ❌ | [trip-cocoon-maldives-26.md](trips/trip-cocoon-maldives-26.md) |
...
| 14 | Sun Siyam Olhuveli | Пакет | 4* | Южный Мале | 576 967 ₽ | ❌ | [trip-sun-siyam-olhuveli-14.md](trips/trip-sun-siyam-olhuveli-14.md) |
...
| 25 | Kudafushi Resort & Spa | Пакет | 5* | Раа | 596 670 ₽ | ❌ | [trip-kudafushi-resort-spa-25.md](trips/trip-kudafushi-resort-spa-25.md) |
| 29 | Niva Dhigali Maldives | Пакет | 5* | Раа | 598 437 ₽ | ❌ | [trip-niva-dhigali-maldives-29.md](trips/trip-niva-dhigali-maldives-29.md) |
| 24 | Hard Rock Hotel Maldives | Пакет | 5* | Южный Мале | 601 530 ₽ | ❌ | [trip-hard-rock-hotel-maldives-24.md](trips/trip-hard-rock-hotel-maldives-24.md) |
...
| 10 | Avani+ Fares Maldives Resort | Пакет | 5* | Атолл Баа | 620 615 ₽ | ❌ | [trip-avani-fares-maldives-resort-10.md](trips/trip-avani-fares-maldives-resort-10.md) |
...
| 30 | Niva Velassaru Maldives | Пакет | 5* | Южный Мале | 678 135 ₽ | ❌ | [trip-niva-velassaru-maldives-30.md](trips/trip-niva-velassaru-maldives-30.md) |
| 28 | The Westin Maldives Miriandhoo Resort | Пакет | 5* | Атолл Баа (биосфера ЮНЕСКО) | 682 729 ₽ | ❌ | [trip-the-westin-maldives-miriandhoo-resort-28.md](trips/trip-the-westin-maldives-miriandhoo-resort-28.md) |
...
- Цены пакетов «перелёт + отель + трансфер» для 27 пакетных отелей из исходного списка **проверены вживую в браузере** (chrome-devtools MCP) на [level.travel](https://level.travel): для каждого отеля открывалась страница с параметрами `nights=7&start_date=2026-11-01&adults=2&from=Moscow-RU` и извлекался минимальный тариф именно «Всё включено» на 7 ночей (не «от X ₽», которое обычно означает завтрак). Проверялось дважды — 21.08.2026 днём и повторно 21.08.2026 вечером, так как level.travel обновляет цены автоматически каждые ~30 минут; в таблице используется последняя (вечерняя) проверка. При повторных запросах числа могут снова отличаться.
